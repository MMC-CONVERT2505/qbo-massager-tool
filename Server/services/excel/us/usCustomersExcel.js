...
module.exports = async function usCustomersExcel(fileId, res) {
  try {
    ...
    if (!records.length) {
      return res.status(404).json({ message: "No US customer data found" });
    }

    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet("United States");
    ...
    console.error("US EXCEL ERROR:", err);
    res.status(500).json({ message: "US Excel export failed" });
  }
};
