...
module.exports = async function usInvoicesExcel({
  ...
  try {
    ...
    if (!records.length) {
      return res.status(404).json({ message: "No Credit Memo data found" });
    }

    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet("Credit Memo");
    ...
    console.error("CREDITMEMO EXCEL ERROR:", err);
    res.status(500).json({ message: "CreditMemo Excel export failed" });
  }
};
