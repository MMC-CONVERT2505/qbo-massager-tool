...
module.exports = async function ukCustomersCSV(fileId, res) {
  try {
    ...
    if (!records.length) {
      return res.status(404).json({ message: "No UK customer data found" });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=united_kingdom_customers.csv"
    );
